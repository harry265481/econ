...
while ($row = mysqli_fetch_array($sqldata, MYSQLI_ASSOC)) {
    ...
    echo '<td>'.$row['ID'].'</td>';
    echo '<td>'.$row['name'].' </td>';
    echo '<td>'. round(getTradeNodeValueByID($link, $row['ID']), 2) .' </td>';
    echo '<td>'. round(getTradeNodePowerByID($link, $row['ID']), 2) .' </td>';
    echo '<form action=node.php method=get>';
